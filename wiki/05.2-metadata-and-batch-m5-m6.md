...
Milestones M5 and M6 delivered schema introspection and transactional batch operations. Both milestones are **complete**. They allow `fm-odata-js` to act as a schema-aware client capable of complex, atomic interactions with FileMaker Server in a single HTTP round-trip.
...
### Public API
- **`FMOData#metadata(opts?)`** — fetches `/$metadata`, parses the CSDL XML into an `ODataMetadata` object, and caches the result. Pass `{ refresh: true }` to bypass the cache.
- **`FMOData#metadataXml(opts?)`** — returns the raw XML string (escape hatch for debugging or forward-compat parsing).

### Data Flow: Metadata Retrieval
```mermaid
graph TD
    subgraph "Client Space"
        A["FMOData Client"] -- "metadata()" --> B["HttpClientContext"]
    end
...
### Public API
- **`FMOData#batch()`** — returns a `Batch` builder instance.
