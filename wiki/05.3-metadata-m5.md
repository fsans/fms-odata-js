...
`fms-odata-js` can fetch and parse the OData CSDL/EDMX schema document exposed by FileMaker Server. The result is a fully typed `ODataMetadata` object that describes every table, field, key, and script in the database. **M5 is complete.**
...
### `FMSOData#metadata(opts?)`

Fetches `GET /<db>/$metadata` (with `Accept: application/xml`), parses the response, and returns an `ODataMetadata` object. Results are cached on the `FMSOData` instance; subsequent calls return the cached object without a network request.
...
sequenceDiagram
    participant App
    participant Client as FMSOData (src/client.ts)
...
- **Caching**: Stored as `_metadataCache` on the `FMSOData` instance. Thread-safe for single-threaded JS environments; `refresh: true` replaces the cache.
...
- `FMSOData#metadataXml()` — correct endpoint, `Accept: application/xml` header
- `FMSOData#metadata()` — full parse, caching (same object reference on repeat call), `refresh: true` forces refetch, `AbortSignal` forwarding
